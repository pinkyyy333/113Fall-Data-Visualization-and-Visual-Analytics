...
(function (d3$1) {
    ...
    const render = (graph) => {
        ...
        link.on("mouseover", function(event, d) {
            d3.select("#tooltip")
                ...
                .html(`來源: ${d.source.name}<br>目標: ${d.target.name}<br>數值: ${d.value}`);
        })
        ...
        const colorScales = {
            "buying": ['hsl(330, 100%, 80%)', 'hsl(330, 90%, 70%)', 'hsl(330, 80%, 60%)', 'hsl(330, 70%, 50%)'], // 粉紅色
            "maintenance": ['hsl(50, 100%, 75%)', 'hsl(50, 90%, 65%)', 'hsl(50, 80%, 55%)', 'hsl(50, 70%, 45%)'], // 黃色
            "doors": ['hsl(140, 100%, 70%)', 'hsl(140, 90%, 60%)', 'hsl(140, 80%, 50%)', 'hsl(140, 70%, 40%)'], // 綠色
            "persons": ['hsl(270, 100%, 75%)', 'hsl(270, 90%, 65%)', 'hsl(270, 80%, 55%)', 'hsl(270, 70%, 45%)'], // 紫色
            "luggage boot": ['hsl(30, 80%, 65%)', 'hsl(30, 70%, 55%)', 'hsl(30, 60%, 45%)', 'hsl(30, 50%, 35%)'], // 橘色偏咖啡
            "safety": ['hsl(210, 100%, 75%)', 'hsl(210, 90%, 65%)', 'hsl(210, 80%, 55%)', 'hsl(210, 70%, 45%)'] // 藍色
        };
    ...
    // 選擇所有的 sankey 節點或邊（例如在這裡是 .node 或 .link）
    d3.selectAll(".node")
    .on("mouseover", function(event, d) {
    // 更新 tooltip 的內容，例如顯示節點的值
    d3.select("#tooltip")
        .style("visibility", "visible")
        .text(`Value: ${d.value}`);
    })
    .on("mousemove", function(event) {
    // 根據滑鼠位置更新 tooltip 的位置
    d3.select("#tooltip")
        .style("top", (event.pageY + 10) + "px")
        .style("left", (event.pageX + 10) + "px");
    })
