...
d3.csv(data_path).then(function (data) {
    ...
    const color = d3.scaleOrdinal()
        .domain(keys)
        .range(["#7DABD0","#F56E4A","#FFDF6F","#FF97A5","#A2D89F","#CAABD8","#D1EAF5"]);

    var blocks = document.getElementById('blocks');
    let html = ""
    for (let i = 0; i < keys.length; i++) {
        html += '<div class="list-group-item" style="background-color:' + color(keys[i]) + '; font-family: Times New Roman, sans-serif; font-size: 16px;">' + keys[i] + '</div>';
    }
    ...
    function render(keys) {
        ...
        const mousemove = function (event, d, i) {
            ...
            Tooltip
                .text(grp) // 在提示框中顯示鍵名
                .style("font-family","Times New Roman") // 設置字型
                .style("font-size", "16px") // 設置字型大小
                .style("fill", "#333333") // 設置字型顏色
        }
